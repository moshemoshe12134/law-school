...
│   │   └── CourseName-transcript-MM-DD.txt
...
│       └── duress_analysis.md
│
└── 07_NOTES/                            # In-class observations & clarifications
    └── YYYY-MM-DD.md                    # Notes on things not in prep/outlines
```
...
#### `original_sources/`
**CRITICAL: This is where original (often unreadable) source files live**

- **Purpose:** Store the original PDFs/DOCX/PPTX/etc so `01_SOURCES/` stays text-first for agents.
- **Structure mirrors `01_SOURCES/`:**
  ```
  01_SOURCES/
  ├── past_outlines/            ← AI-readable (.md/.txt)
  ├── past_exams/               ← AI-readable (.md/.txt)
  └── original_sources/         ← Originals (PDF/DOCX/PPTX/etc)
      ├── past_outlines/
      │   └── Outline_2023.pdf
      └── past_exams/
          └── Exam_2023.pdf
...
**Conversion tools:**
- Use OCR scripts in `law-school-common/04_SCRIPTS/` to convert
- Store converted files in the corresponding `01_SOURCES/` folder with the same base name, using `.md` or `.txt`

**AI Agent Rule:**
- **ALWAYS start in `01_SOURCES/` (text-first)** when looking for outlines/exams/readings
- Only look in `01_SOURCES/original_sources/` if no converted version exists, and then tell the user conversion is needed

#### `converted_md/` (deprecated)
- Some older folders may still contain `converted_md/`; converted files should be promoted into `01_SOURCES/` and not left here.

#### `past_exams/`
- PDF files of past exams
- OCR'd text versions
- Organized by year: `2023_Spring_Exam.pdf`
- **Converted versions:** See `01_SOURCES/past_exams/` for readable .md/.txt files (originals live in `01_SOURCES/original_sources/past_exams/`)
...
- **Converted versions:** See `01_SOURCES/past_outlines/` for readable .md/.txt files (originals live in `01_SOURCES/original_sources/past_outlines/`)
...
**AI Agent Rule:** Transcripts are CRITICAL resources for creating review docs. Always check raw/ first, then processed/ for diarized versions.

**Subfolders:**

#### `raw/`
- **Location:** Direct downloads from Echo360/recording platform
- **Format:** `.txt` files (raw transcription from platform)
- **Naming pattern:** `{CourseName}-transcript-MM-DD.txt`
  - Examples: 
    - `Property-transcript-01-27.txt`
    - `CrimLaw-transcript-01-22.txt`
- **Do not edit** - keep original for error detection
- **AI Agent Note:** This is the PRIMARY source for transcript content. Check here first when creating review docs.

#### `processed/`
- **Purpose:** Cleaned/enhanced versions when available
- **Two types:**
  1. **Basic cleaned:** Transcription errors fixed, legal terms corrected
  2. **Diarized:** Speaker-labeled transcripts (especially valuable for CrimLaw and other discussion-heavy classes)
  
- **Format:** `.md` files (markdown) or `.txt` files
- **Naming patterns:**
  - Basic: `YYYY-MM-DD_classNN_clean.md`
  - Diarized: `YYYY-MM-DD_classNN_diarized.md`
  
- **Enhancements may include:**
  - Transcription error fixes (legal terms, case names, common mishearings)
  - Speaker labels (PROFESSOR, STUDENT 1, STUDENT 2, etc.)
  - Timestamps
  - Section headers
  - Paragraph breaks for readability
  
- **When diarization matters:**
  - **CrimLaw:** Class discussion is especially valuable—need to distinguish professor vs. student arguments
  - **Socratic-heavy classes:** Tracking who said what helps identify professor's preferred reasoning
  - **Policy debates:** Understanding back-and-forth exchanges
  
- **AI Agent Note:** 
  - Check if processed/ has a diarized or cleaned version before using raw/
  - If processed version exists, prefer it (more accurate, better structured)
  - If only raw/ exists, note transcription quality issues in review QC section

**Example structure:**
```
03_TRANSCRIPTS/
├── raw/
│   ├── Property-transcript-01-20.txt
│   ├── Property-transcript-01-22.txt
│   └── Property-transcript-01-27.txt
└── processed/
    ├── 2026-01-20_class01_clean.md      (basic cleanup)
    └── 2026-01-22_class02_diarized.md   (speaker labels added)
```
...
**Purpose:** In-class observations and clarifications that don't appear in prep docs or past outlines

**Use case:** During class, you notice:
- Professor's unique take on a doctrine (different from casebook or prep)
- Clarifications on ambiguous points
- Connections between topics not obvious from prep
- Exam hints or emphasis signals
- Questions to follow up on
- Personal insights or "aha" moments

**Format:** Simple dated notes, informal

```
07_NOTES/
├── 2026-01-20.md    # Jan 20 class notes
├── 2026-01-22.md    # Jan 22 class notes
└── 2026-01-27.md    # Jan 27 class notes
```

**Each note file contains:**
- Quick bullet points
- Key quotes from professor
- Observations about what stuck out
- Things that differed from expectations
...
- party chooses option that benefits themselves (internal) but not external effects. 
- ranchers cannot internalize the benefits from refraining from over-grazing.
- result: all grass is eaten and there is no grass left.

Knowledge problem:
- F.a. hayak. use of knowledge in society.
- central planners cannot have enough information to make optimal decisions. 

[...additional notes...]
```

**Integration with review workflow:**
- When creating review docs, AI will check for corresponding notes file
- Notes get incorporated into "Class Notes Integration" section of review
- Helps capture details that might not appear in transcript

**Key distinction from other folders:**
- **02_PREP:** What you prepare BEFORE class
- **03_TRANSCRIPTS:** What professor said IN class (verbatim)
- **04_REVIEWS:** What you analyze AFTER class (structured review)
- **07_NOTES:** What YOU observed DURING class (personal observations)
